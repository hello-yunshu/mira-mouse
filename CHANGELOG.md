...
- 修复宿主生产插件信任库中的密钥编号与公钥映射，并保留 `001`、`002` 两代签名插件的兼容性。
- 新增宿主级发布回归测试，直接验证所有默认随包插件的锁定摘要、发布密钥、签名、插件 ID 与版本。
- 将 Razer 插件升级至 `0.3.1`，改用当前共享 Package Format CLI 生成可由宿主加载的不可变正式资产。
- 完成三平台构建、自动更新、本地 AI 稳定指针与 Homebrew 同步所需的正式发布配置。

## 1.0.0-rc.1 - 2026-07-29

- 完成 Iteration 008–011 的启动生命周期、声明式 UI、设备电量、滚动状态与通知行为收口。
- 将本地 AI 运行时升级至 RillML 1.0.0，并补齐预测、签名索引与故障回退验证。
- 将插件打包切换到独立的 `mira-plugin-cli 1.0.0`，生产插件使用不可变的 per-plugin Release。
- RC Release 标记为 prerelease，不更新 Homebrew，不移动稳定版应用或本地 AI 指针。
