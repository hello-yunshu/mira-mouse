...
- `src-tauri/resources/plugins/mira-amaster-1.2.0.mira-plugin`：重新打包的 amaster 插件（1.2.0，生产签名）。
...
- `crates/mira-core/src/lib.rs`：扩展 `DeviceSnapshot`（`DpiStage`、`dpiStages`、`evidence`）。
- `src/App.tsx`、`src/types.ts`：前端使用真实 DPI 档位和证据等级。
- `plugins.lock.json`：更新 amaster 插件版本至 1.2.0 及 SHA-256。
- `src-tauri/tauri.conf.json`：资源路径指向 1.2.0 产物。
...
```
sha256: 693e234a87320d9ffc23765f9b45a219a85146d5592a0ccc58e659422f2d9ee0
plugin: mira.amaster v1.2.0 signature_verified=true
...
```bash
cargo test --workspace          # ok (8 tests)
...
- 文件：`src-tauri/resources/plugins/mira-amaster-1.2.0.mira-plugin`
- SHA-256：`693e234a87320d9ffc23765f9b45a219a85146d5592a0ccc58e659422f2d9ee0`
- 签名：Ed25519 生产签名，publisherKeyId `mira-plugins-2026-001`，公钥 `eb80fdde2dc7ba507b6c8afbbf5a7de82e6219967edf1914ddb979d5601d39b3`
- 验证：`node scripts/verify-test.mjs` 通过
